using OpenCvSharp;
namespace Pierre.Darcas.FeatureMatching;
...
public class ObjectDetection
{
    public async Task<IList<ObjectDetectionResult>> DetectObjectInScenes(byte[] objectImageData, IList<byte[]> imagesSceneData)
    {
        ...
        List<ObjectDetectionResult> objectDetectionResults = new List<ObjectDetectionResult>();
        foreach (var image in imagesSceneData)
        {
            var objectDetectionResult = await Task.Run(() =>DetectObjectInScene(objectImageData, image));
